...
--[[
    ...
    The following return information about one of the four layouts above:
        ...
        rematchRedux.layout:GetLayout(C.CURRENT) -- returns the name of the layout used for one of the four layouts above (*)
        rematchRedux.layout:GetLayout(layoutName) -- returns the layoutName if valid and adjusts it to suit the current mode if not
        rematchRedux.layout:GetDefinition(C.CURRENT) -- returns the full layout definition in table form
        rematchRedux.layout:GetMode(C.CURRENT) -- returns the mode (0=minimized, 1=single panel, 2=dual panel, 3=triple panel)
        rematchRedux.layout:GetView(C.CURRENT) -- returns the view ("pets", "teams", "queue", "options")
    ...
    The following can be used to alter the layout:
        ...
        rematchRedux.layout:ChangeMode(mode) -- changes the mode of the current layout to the given mode and calls a Configure
        rematchRedux.layout:ChangeView(view) -- changes the view of the current layout to the given view and calls a Configure
]]

-- on login, set up layout definitions
rematchRedux.events:Register(rematchRedux.layout,"PLAYER_LOGIN",function(self)
    -- register the layouts defined in definitions.lua
    for _,layout in ipairs(rematchRedux.layout.definitions) do
        rematchRedux.layout:Register(layout)
    end
    -- done with definitions, safe to delete
    rematchRedux.layout.definitions = nil
end)

-- registers a layout in allLayouts and returns the layout's name
function rematchRedux.layout:Register(layout)
    ...
    for _,info in ipairs(layout.panels) do
        ...
        assert(info[1] and rematchRedux.frame[info[1]],"Layout panel nil parentKey in "..layoutName)
        assert(info[3] and rematchRedux.frame[info[3]],"Layout panel nil relativeTo1 in "..layoutName)
        assert(info[8] and rematchRedux.frame[info[8]],"Layout panel nil relativeTo2 in "..layoutName)
    end
    ...
    for _,info in ipairs(layout.panels) do
        knownPanels[rematchRedux.frame[info[1]]] = true
        allLayouts[layoutName].panelsUsed[rematchRedux.frame[info[1]]] = true
    end

    -- register the layout to its tab (if any)
    rematchRedux.panelTabs:Register(layoutName)
...
function rematchRedux.layout:GetLayout(layout)
...
function rematchRedux.layout:GetDefinition(layout)
    return allLayouts[rematchRedux.layout:GetLayout(layout)]
end

-- returns the mode for the CURRENT, STANDALONE, MAXIMIZED, JOURNAL or named layout
function rematchRedux.layout:GetMode(layout)
    return rematchRedux.layout:GetDefinition(layout).mode
end

-- returns the view for the CURRENT, STANDALONE, MAXIMIZED, JOURNAL or named layout
function rematchRedux.layout:GetView(layout)
    return rematchRedux.layout:GetDefinition(layout).view
end

-- returns the subview for the CURRENT, STANDALONE, MAXIMIZED, JOURNAL or named layout
function rematchRedux.layout:GetSubview(layout)
    return rematchRedux.layout:GetDefinition(layout).subview
end
...
function rematchRedux.layout:ChangeMode(mode)
    ...
    if def.view and mode then
        rematchRedux.frame:Configure(rematchRedux.layout:GetLayout(mode.."-"..def.view))
    end
end

-- changes the current layout to a different view of the same mode
function rematchRedux.layout:ChangeView(view)
    local def = rematchRedux.layout:GetDefinition(C.CURRENT)
    ...
    if def.mode and view then
        rematchRedux.frame:Configure(rematchRedux.layout:GetLayout(def.mode.."-"..view))
    end
end

-- intended to turn a subview on and off of the current layout; subview is nil to turn it off
function rematchRedux.layout:ChangeSubview(subview)
    local def = rematchRedux.layout:GetDefinition(C.CURRENT)
    if def.subview==subview then
        return
    end
    if not subview then
        rematchRedux.frame:Configure(rematchRedux.layout:GetLayout(def.mode.."-"..def.view))
    else
        rematchRedux.frame:Configure(rematchRedux.layout:GetLayout(def.mode.."-"..def.view.."-"..subview))
    end
end

-- hides all panels that don't belong to the given definition
function rematchRedux.layout:HidePanels(def)
...
-- a more forceful version of ChangeView, it summons the RematchRedux window (in preferred mode if not already up) and
...
function rematchRedux.layout:SummonView(view)
    -- summon window if it's not already visible
    if not rematchRedux.frame:IsVisible() then
    ...
    if rematchRedux[panel] and not rematchRedux[panel]:IsVisible() then
        if rematchRedux.layout:GetMode()==0 then -- in minimized mode, switch to a maximized mode with the view
            local mode = rematchRedux.layout:GetMode(C.MAXIMIZED)
            rematchRedux.frame:Configure(mode.."-"..view)
        else -- not minimized, simpler switch to the view
            rematchRedux.layout:ChangeView(view)
        end
    end
end
