...
rematchRedux.scriptFilter = {}
local settings = rematchRedux.settings

rematchRedux.events:Register(rematchRedux.scriptFilter,"PLAYER_LOGIN",function(self)
    self.petInfo = rematchRedux.petInfo:Create() -- creating a separate petInfo for scripting purposes
    self.altInfo = rematchRedux.petInfo:Create() -- create an alternate one too if needed
    ...
    rematchRedux.dialog:Register("ScriptFilterDialog",{
        ...
        refreshFunc = function(self,info,subject,firstRun)
            ...
            if firstRun then
                ...
                rematchRedux.dialog.AcceptButton:SetEnabled(subject and true)
                rematchRedux.dialog.OtherButton:SetEnabled(subject and true)
            end
        end,
        ...
        otherFunc = function(self,info,subject)
            ...
            rematchRedux.dialog:ChangeLayout("Results")
            -- run the filter (any failures will end up in HandleError and do a Feedback:Set("failure",message))
            rematchRedux.filters:Set("Script","Code",self.MultiLineEditBox:GetText())
        ...
        changeFunc = function(self,info,subject)
            ...
            rematchRedux.dialog.AcceptButton:SetEnabled(hasCode and self.EditBox:GetText():trim():len()>0)
            rematchRedux.dialog.OtherButton:SetEnabled(hasCode)
        end,
        acceptFunc = function(self,info,subject)
            ...
            rematchRedux.timer:Start(0,function()
                rematchRedux.filters:Set("Script","Code",settings.ScriptFilters[subject][2])
                rematchRedux.petsPanel:Update()
            end)
        end,
    })

    rematchRedux.dialog:Register("DeleteScriptFilterDialog",{
    ...
    rematchRedux.dialog:Register("ScriptFilterErrorDialog",{
...
function rematchRedux.scriptFilter:SetupEnvironment()
    ...
    self.environment = {
        ...
        -- iterators
        AllSpeciesIDs=rematchRedux.roster.AllSpecies, AllPetIDs=rematchRedux.roster.AllOwnedPets, AllPets=rematchRedux.roster.AllPets,
...
function rematchRedux.scriptFilter:CleanupEnvironment()
...
function rematchRedux.scriptFilter:HandleError(message)
    message = (message or ""):gsub("^.-string \"\"%]%:",L["line "]) -- strip out the error stack before the error
    if rematchRedux.dialog:GetOpenDialog()=="ScriptFilterDialog" then -- script filter dialog up; display error there
        rematchRedux.dialog.Canvas.Feedback:Set("failure",message)
        rematchRedux.dialog:ChangeLayout("Results")
    else -- script filter dialog isn't up, display a separate dialog
        rematchRedux.dialog:ShowDialog("ScriptFilterErrorDialog",message)
    end
...
function rematchRedux.scriptFilter:AllAbilities()
    local i = 0
    local petInfo = rematchRedux.petInfo:Fetch(self)
    return function()
        i = i + 1
...
function rematchRedux.scriptFilter:GetBreed()
    return rematchRedux.petInfo:Fetch(self).breedID
end
-- legacy: gets numeric source of a pet; self is petID passed in GetSource(petID); use petInfo.sourceID instead
function rematchRedux.scriptFilter:GetSource()
    return rematchRedux.petInfo:Fetch(self).sourceID
end
-- legacy: gets whether pet is leveling; self is petID passed in IsPetLeveling(petID); use petInfo.isLeveling instead
function rematchRedux.scriptFilter:IsPetLeveling()
    return rematchRedux.petInfo:Fetch(self).isLeveling
end

-- called by RunFilters to evaluate whether the petInfo should list
function rematchRedux.scriptFilter:Evaluate(petInfo)
