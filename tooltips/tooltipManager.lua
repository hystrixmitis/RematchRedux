...
--[[
    ...
    The function rematchRedux.tooltipManager:AddBehavior(tooltip) will add this behavior to any frame given as a tooltip,
...
function rematchRedux.tooltipManager:AddBehavior(tooltip)
    ...
    tooltip.Show = function(self,now)
        ...
        if rematchRedux.timer:IsRunning(timer) then
            rematchRedux.timer:Stop(timer)
        end
        if now then
        ...
        elseif settings.TooltipBehavior==C.MOUSE_SPEED_SLOW then
            rematchRedux.timer:Start(C.CARD_MANAGER_DELAY_SLOW,timer)
        elseif settings.TooltipBehavior==C.MOUSE_SPEED_NORMAL then
            rematchRedux.timer:Start(C.CARD_MANAGER_DELAY_NORMAL,timer)
