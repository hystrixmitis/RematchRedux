...
rematchRedux.petMenu = {}
local pm = rematchRedux.petMenu

rematchRedux.events:Register(rematchRedux.petMenu,"PLAYER_LOGIN",function(self)
    ...
    rematchRedux.menus:Register("PetMenu",menu)
	...
	rematchRedux.menus:Register("SetPetMarker",setPetMarkerMenu)
    ...
    local renameDialog = {
        ...
        refreshFunc = function(self,info,subject,firstRun)
            ...
            if firstRun then
                ...
                rematchRedux.dialog.OtherButton:SetEnabled(petInfo.customName and true or false)
            end
        end,
        changeFunc = function(self,info,subject)
            ...
            rematchRedux.dialog.AcceptButton:SetEnabled(canAccept)
            rematchRedux.dialog:ChangeLayout(showWarning and "Invalid" or "Default")
        end,
    ...
    rematchRedux.dialog:Register("RenameDialog",renameDialog)
    ...
    local hideDialog = {
        ...
        refreshFunc = function(self,info,subject,firstRun)
            self.Pet:Fill(subject)
            local speciesName = rematchRedux.petInfo:Fetch(subject).speciesName
    ...
    rematchRedux.dialog:Register("HidePetDialog",hideDialog)
    ...
    local releaseDialog = {
        ...
        refreshFunc = function(self,info,subject,firstRun)
            ...
            if rematchRedux.petInfo:Fetch(subject).inTeams then
    ...
    rematchRedux.dialog:Register("ReleasePetDialog",releaseDialog)
    ...
    local cageDialog = {
        ...
        refreshFunc = function(self,info,subject,firstRun)
            ...
            if rematchRedux.petInfo:Fetch(subject).inTeams then
                ...
                rematchRedux.dialog:ChangeLayout("InTeams")
            end
        end,
        acceptFunc = function(self,info,subject)
            if self.CheckButton:GetChecked() then
    ...
    rematchRedux.dialog:Register("CagePetDialog",cageDialog)

end)

function rematchRedux.petMenu:GetPetName(petID)
    return rematchRedux.petInfo:Fetch(petID).formattedName
end

function rematchRedux.petMenu:SummonOrDismissText(petID)
    return C_PetJournal.GetSummonedPetGUID()==petID and PET_ACTION_DISMISS or SUMMON
end

function rematchRedux.petMenu:NotOwnedPet(petID)
    local petInfo = rematchRedux.petInfo:Fetch(petID)
    return not (petInfo.isOwned and petInfo.idType=="pet")
end

function rematchRedux.petMenu:NotObtainablePet(petID)
    return not rematchRedux.petInfo:Fetch(petID).isObtainable
end

function rematchRedux.petMenu:SummonOrDismissFunc(petID)
    C_PetJournal.SummonPetByGUID(petID)
end

function rematchRedux.petMenu:RenamePetFunc(petID)
    rematchRedux.dialog:ShowDialog("RenameDialog",petID)
end

function rematchRedux.petMenu:SetOrRemoveFavoriteText(petID)
    return rematchRedux.petInfo:Fetch(petID).isFavorite and BATTLE_PET_UNFAVORITE or BATTLE_PET_FAVORITE
end

function rematchRedux.petMenu:SetOrRemoveFavoriteFunc(petID)
    C_PetJournal.SetFavorite(petID,rematchRedux.petInfo:Fetch(petID).isFavorite and 0 or 1)
    rematchRedux.filters:ForceUpdate()
    rematchRedux.frame:Update()
end

function rematchRedux.petMenu:HideOrShowPetText(petID)
    local speciesID = rematchRedux.petInfo:Fetch(petID).speciesID
    return (speciesID and settings.HiddenPets[speciesID]) and L["Show Pet"] or L["Hide Pet"]
end

function rematchRedux.petMenu:DontAllowHiddenPets(petID)
    return not settings.AllowHiddenPets
end

-- function for Hide/Show Pet
function rematchRedux.petMenu:HideOrShowPetFunc(petID)
    local speciesID = rematchRedux.petInfo:Fetch(petID).speciesID
    if speciesID then
        ...
        elseif not settings.DontConfirmHidePets then -- if Don't Ask When Hiding Pets unchecked, ask
            rematchRedux.dialog:ShowDialog("HidePetDialog",petID)
...
function rematchRedux.petMenu:GetMarkerName()
...
function rematchRedux.petMenu:GetIconCoords()
...
function rematchRedux.petMenu:PetNotTradable(petID)
    local petInfo = rematchRedux.petInfo:Fetch(petID)
    return not petInfo.isOwned or not rematchRedux.petInfo:Fetch(petID).isTradable
end

-- returns true if pet is injured or slotted and can't be caged
function rematchRedux.petMenu:CantCagePet(petID)
    local petInfo = rematchRedux.petInfo:Fetch(petID)
    return petInfo.isInjured or petInfo.isSlotted
end

-- returns the tooltip explaining why cage is disabled
function rematchRedux.petMenu:CageDisableReason(petID)
    local petInfo = rematchRedux.petInfo:Fetch(petID)
...
function rematchRedux.petMenu:CagePetFunc(petID)
    if not rematchRedux.petInfo:Fetch(petID).inTeams and settings.DontConfirmCaging then
        C_PetJournal.CagePetByID(petID)
    else
        rematchRedux.dialog:ShowDialog("CagePetDialog",petID)
    end
end

-- returns true if pet can't be released
function rematchRedux.petMenu:PetNotReleasable(petID)
    local petInfo = rematchRedux.petInfo:Fetch(petID)
    return not petInfo.isOwned or not C_PetJournal.PetCanBeReleased(petID)
end

function rematchRedux.petMenu:PetIsSlotted(petID)
    return rematchRedux.petInfo:Fetch(petID).isSlotted
end

function rematchRedux.petMenu:ReleasePetFunc(petID)
    rematchRedux.dialog:ShowDialog("ReleasePetDialog",petID)
end

function rematchRedux.petMenu:FindSimilarFunc(petID)
    local speciesID = rematchRedux.petInfo:Fetch(petID).speciesID
    if speciesID then
...
function rematchRedux.petMenu:FindMovesetFunc(petID)
    local speciesID = rematchRedux.petInfo:Fetch(petID).speciesID
    if speciesID then
        rematchRedux.filters:SetMovesetFilter(speciesID)
        rematchRedux.filters:ForceUpdate()
        rematchRedux.petsPanel:Update()
    end
end

function rematchRedux.petMenu:SetPetMarker(petID)
    local speciesID = rematchRedux.petInfo:Fetch(petID).speciesID
    if speciesID then
...
function rematchRedux.petMenu:ShowNotes(petID)
...
function rematchRedux.petMenu:PetCantLevel(petID)
    return not rematchRedux.queue:PetIDCanLevel(petID)
end

function rematchRedux.petMenu:PetLevelingOrCant(petID)
    return not rematchRedux.queue:PetIDCanLevel(petID) or rematchRedux.petInfo:Fetch(petID).isLeveling
end

function rematchRedux.petMenu:ToggleQueueText(petID)
    return rematchRedux.queue:IsPetLeveling(petID) and L["Remove from Leveling Queue"] or L["Add to Leveling Queue"]
end

-- adds or removes petID to/from the leveling queue
function rematchRedux.petMenu:ToggleQueueFunc(petID)
...
function rematchRedux.petMenu:StartQueueFunc(petID)
    rematchRedux.queue:InsertPetID(petID,1)
    rematchRedux.queue:Process()
    rematchRedux.queue:BlingPetID(petID)
end

function rematchRedux.petMenu:MovePetInQueue(petID)
    C_PetJournal.PickupPet(petID)
end

function rematchRedux.petMenu:NotInTeam(petID)
    local numTeams = rematchRedux.petInfo:Fetch(petID).numTeams
    return not numTeams or numTeams==0
end

function rematchRedux.petMenu:ListTeams(petID)
...
function rematchRedux.petMenu:RenameMarker()
	rematchRedux.dialog:ShowDialog("RenamePetMarkerDialog",self.key)
end

function rematchRedux.petMenu:IsMarkerChecked(petID)
