...
rematchRedux.dragFrame = RematchReduxDragFrame

--[[ for dragging teams, groups (and maybe targets) ]]

rematchRedux.events:Register(rematchRedux.dragFrame,"PLAYER_LOGIN",function(self)
    ...
    rematchRedux.dialog:Register("CombineGroups",{
        ...
        refreshFunc = function(self,info,subject,firstRun)
            ...
            local fromName = rematchRedux.utils:GetFormattedGroupName(subject.fromID)
            local toName = rematchRedux.utils:GetFormattedGroupName(subject.toID)
        ...
        acceptFunc = function(self,info,subject)
            if subject.fromID~=subject.toID and rematchRedux.savedGroups[subject.fromID] and rematchRedux.savedGroups[subject.toID] then
                ...
                for teamID,team in rematchRedux.savedTeams:AllTeams() do
                    if team.groupID==subject.fromID then
                        ...
                        if settings.EchoTeamDrag then
                            rematchRedux.utils:Write(rematchRedux.utils:GetFormattedTeamName(teamID),"moved to",rematchRedux.utils:GetFormattedGroupName(subject.toID))
                        end
                    end
                end
                ...
                if not settings.DontDeleteOnCombine and subject.fromID~="group:favorites" and subject.fromID~="group:none" then
                    rematchRedux.savedGroups:Delete(subject.fromID)
                end
                -- doing an immediate TeamsChanged
                rematchRedux.savedTeams:TeamsChanged(true) -- true will update UI immediately
                rematchRedux.teamsPanel.List:BlingData(subject.toID) -- bling the group teams were moved to
            end
        end
    })
...
function rematchRedux.dragFrame:GetCursorInfo()
    return self.cursorType,self.cursorID
end

-- returns a C.DRAG_DIRECTION_PREV or C.DRAG_DIRECTION_NEXT
function rematchRedux.dragFrame:GetCursorDirection()
    if self.GlowFrame.GlowLine:IsVisible() then
        return self.GlowFrame.GlowLine.direction
    end
end

function rematchRedux.dragFrame:OnShow()
    SetCursor("ITEM_CURSOR")
    --self:GetScript("OnUpdate")(self,0)
end

function rematchRedux.dragFrame:OnHide()
    self.cursorType = nil
    self.cursorID = nil
    SetCursor(nil)
    self.GlowFrame:Hide()
    rematchRedux.teamsPanel.List:UnlockHeaders()
    rematchRedux.teamsPanel.List:Select("Moving")
    rematchRedux.teamsPanel.List.needsRefresh = true
    rematchRedux.events:Unregister(self,"GLOBAL_MOUSE_UP")
end

function rematchRedux.dragFrame:OnUpdate(elapsed)
    if GetCursorInfo() then -- something real was picked up on cursor
        self:Hide()
    elseif self:GetCursorInfo() then
        if not rematchRedux.teamsPanel:IsVisible() then -- if not on teams panel, drop whatever group/team on cursor
            self:ClearCursor()
        else
...
function rematchRedux.dragFrame:OnClick(button)
    self:ClearCursor()
end
...
function rematchRedux.dragFrame:GLOBAL_MOUSE_UP()
    local focus = GetMouseFoci()[1]
    if focus then
        -- if mouse is over teamsPanel(GlowFrame) then any team/group combination safe to handle
        if self.GlowFrame:IsMouseOver() and rematchRedux.teamsPanel:IsVisible() and (focus.groupID or focus.teamID or focus==rematchRedux.teamsPanel.List.CaptureButton) then
            self:HandleReceiveDrag(focus)
        end
        -- but if mouse is over teamTabs it can only be a team on the cursor
        if rematchRedux.teamTabs:IsMouseOver() and rematchRedux.teamTabs:IsVisible() and focus.groupID and self:GetCursorInfo()==C.CURSOR_TYPE_TEAM then
            self:HandleReceiveDrag(focus)
        end
    end
    rematchRedux.events:Unregister(self,"GLOBAL_MOUSE_UP")
...
function rematchRedux.dragFrame:ClearCursor()
    self:Hide()
    rematchRedux.teamsPanel.List:Select("Moving")
end

-- picks up a groupID onto the cursor; fromDrag is true if it was picked up from OnDragStart
function rematchRedux.dragFrame:PickupGroup(groupID,fromDrag)
    ClearCursor()
    if rematchRedux.savedGroups[groupID] then
        ...
        self.Group.Name:SetText(rematchRedux.utils:GetFormattedGroupName(groupID))
        ...
        self.Group.Icon:SetTexture(rematchRedux.savedGroups[groupID].icon or C.EMPTY_ICON)
        self.Team:Hide()
        self.Group:Show()

        -- if not in teams tab, go to it
        if rematchRedux.layout:GetMode()~=0 and rematchRedux.layout:GetView()~="teams" then
            rematchRedux.layout:ChangeView("teams")
        end
        -- if searching, clear search
        if rematchRedux.teamsPanel.List:IsSearching() then
            rematchRedux.teamsPanel.Top.SearchBox.Clear:Click()
        end
        -- collapse any expanded headers and lock them to prevent expanding
        rematchRedux.teamsPanel.List:CollapseAllHeaders()
        rematchRedux.teamsPanel.List:LockHeaders()

        self:Show() -- show dragFrame to begin dragging
        self:ShowGlowFrame(rematchRedux.teamsPanel.List)
        ...
        rematchRedux.teamsPanel.List:Select("Moving",groupID)
    end
end

-- picks up a teamID onto the cursor; fromDrag is true if it was picked up from OnDragStart
function rematchRedux.dragFrame:PickupTeam(teamID,fromDrag)
    ClearCursor()
    if rematchRedux.savedTeams[teamID] then
        rematchRedux.utils:HideWidgets()
        rematchRedux.dialog:Hide()
        ...
        for i=1,3 do
            local petID = rematchRedux.savedTeams[teamID].pets[i]
        ...
        if rematchRedux.teamsPanel.List:IsSearching() then
            ...
            rematchRedux.teamsPanel.List:CollapseAllButData(teamID)
        end
        ...
        rematchRedux.teamsPanel.List:Select("Moving",teamID)
    end
end

--[[ glow frame ]]

function rematchRedux.dragFrame:ShowGlowFrame(parent)
...
function rematchRedux.dragFrame.GlowFrame:OnUpdate(elapsed)
    local cursorType,cursorID = rematchRedux.dragFrame:GetCursorInfo()
    local focus = GetMouseFoci()[1]
    ...
    if cursorType==C.CURSOR_TYPE_GROUP and isMouseOver then
        if focus and focus.groupID then
            ...
            if rematchRedux.dragFrame:IsCombineGroupKeyUsed() then
        ...
        elseif focus==rematchRedux.teamsPanel.List.CaptureButton then -- if over capturebutton, then move glow to last button
            if not rematchRedux.dragFrame:IsCombineGroupKeyUsed() then -- make sure combine key not down over capture button
                local lastListButton = rematchRedux.teamsPanel.List:GetLastListButton()
...
function rematchRedux.dragFrame:HandleReceiveDrag(button,mouseButton)
    ...
    if cursorType==C.CURSOR_TYPE_GROUP then -- a groupID is on the cursor
        if button.groupID then -- dropping group onto group
            ...
            rematchRedux.dragFrame:ReceiveGroupToGroup(cursorID,button.groupID,direction)
            return true
        elseif direction==C.DRAG_DIRECTION_END then -- dropping group onto capture area
            PlaySound(C.SOUND_DRAG_STOP)
            rematchRedux.dragFrame:ReceiveGroupToCapture(cursorID)
            return true
        end
    elseif cursorType==C.CURSOR_TYPE_TEAM then -- a teamID is on the cursor
        if button.groupID then -- dropping team onto group
            PlaySound(C.SOUND_DRAG_STOP)
            rematchRedux.dragFrame:ReceiveTeamToGroup(cursorID,button.groupID)
            return true
        elseif button.teamID then -- dropping team onto team
            PlaySound(C.SOUND_DRAG_STOP)
            rematchRedux.dragFrame:ReceiveTeamToTeam(cursorID,button.teamID,direction)
            return true
        end
    end
...
function rematchRedux.dragFrame:ReceiveGroupToGroup(cursorID,groupID,direction)
    if cursorID==groupID then
        -- do nothing, trying to move a group before/after itself
        rematchRedux.dragFrame:ClearCursor()
        rematchRedux.teamsPanel.List:BlingData(cursorID)
    elseif rematchRedux.dragFrame:IsCombineGroupKeyUsed() then -- if combining groups, show dialog
        rematchRedux.dialog:ShowDialog("CombineGroups",{fromID=cursorID,toID=groupID})
        rematchRedux.dragFrame:ClearCursor()
    elseif direction then
        -- remove cursorID from settings.GroupOrder
        rematchRedux.utils:TableRemoveByValue(settings.GroupOrder,cursorID)
...
function rematchRedux.dragFrame:ReceiveGroupToCapture(cursorID)
    self:ReceiveGroupToGroup(cursorID,nil,C.DRAG_DIRECTION_END)
end

-- dropping teamID(cursorID) onto groupID
function rematchRedux.dragFrame:ReceiveTeamToGroup(cursorID,groupID,noBling)
    local teamName = rematchRedux.utils:GetFormattedTeamName(cursorID)
    local team = rematchRedux.savedTeams[cursorID]
    ...
    if not noBling then
        rematchRedux.teamsPanel.List:BlingData(groupID)
    end
    if settings.EchoTeamDrag and teamMoved then
        rematchRedux.utils:Write(teamName,"moved to",rematchRedux.utils:GetFormattedGroupName(groupID))
    end
end

-- dropping teamID(cursorID) before/after a teamID
function rematchRedux.dragFrame:ReceiveTeamToTeam(cursorID,teamID,direction)
    local team = rematchRedux.savedTeams[cursorID]
    local group = rematchRedux.savedGroups[team.groupID]

    if cursorID==teamID then
        rematchRedux.dragFrame:ClearCursor() -- team being moved before/after itself, do nothing
    else
        ...
        -- remove cursorID from settings.GroupOrder
        rematchRedux.utils:TableRemoveByValue(group.teams,cursorID)
        ...
        if teamID then
            ...
            rematchRedux.dragFrame:ClearCursor()
            rematchRedux.teamsPanel:Update() -- don't need to do a TeamsChanged really
        end
    end
    rematchRedux.teamsPanel.List:BlingData(cursorID)
end
...
function rematchRedux.dragFrame:IsCombineGroupKeyUsed(key)
