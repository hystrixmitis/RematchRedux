...
function rematchRedux.randomPets:PickRandomPetID(info)
    ...
    for petID in rematchRedux.roster:AllOwnedPets() do
        if not info.excludePetIDs or not info.excludePetIDs[petID] then
            local weight = rematchRedux.randomPets:GetPetIDWeight(petID,info)
...
function rematchRedux.randomPets:GetPetIDWeight(petID,info)
    if not info then
        info = {}
    end
    ...
    if petInfo.canBattle and not petInfo.isDead and petInfo.speciesID~=1426 and petInfo.isSummonable then
        ...
        if petInfo.level then
            ...
            if info.levelable and settings.QueueRandomWhenEmpty and not settings.QueueRandomMaxLevel then
                ...
                if petInfo.isSummonable and rematchRedux.queue:PetIDCanLevel(petID) then
                    levelWeight = rematchRedux.preferences:IsPetPreferred(petID) and 25 or 1
...
function rematchRedux.randomPets:PickCounter(speciesID,info)
    ...
    if not petInfo.speciesID then -- if not a valid speciesID, then pick a totally random one
        local petID = rematchRedux.randomPets:PickRandomPetID(info)
        return petID, rematchRedux.petTags:Create(petID)
    end
    ...
    return petID, rematchRedux.petTags:Create(petID,counterAbilities[1] or 0,counterAbilities[2] or 0,counterAbilities[3] or 0)
end
...
function rematchRedux.randomPets:BuildCounterTeam(npcID)
    if not npcID then -- if no npcID given, use recent target
        npcID = rematchRedux.targetInfo.recentTarget
    end
    rematchRedux.savedTeams:Reset("counter")
    wipe(chosenPets)
    local team = rematchRedux.savedTeams.counter
    ...
    local opponents = rematchRedux.targetInfo:GetNpcPets(npcID)
    for slot=1,3 do
        local counterPetID
        if numPets==3 then
        ...
        local speciesID = counterPetID and rematchRedux.petInfo:Fetch(counterPetID).speciesID
        local petID,petTag = rematchRedux.randomPets:PickCounter(speciesID,{excludePetIDs=chosenPets})
