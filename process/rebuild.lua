...
local function rebuildTeamID(teamID)
    ...
    for i=1,3 do
        local petID = team.pets[i]
        local petInfo = rematchRedux.rebuild.petInfo:Fetch(petID)
        if team.tags[i] and ((petInfo.idType=="species" and petInfo.count>0) or (petInfo.idType=="pet" and not petInfo.isValid)) then
            local newPetID = rematchRedux.petTags:FindPetID(team.tags[i],excludePetIDs)
...
local function rebuildTeam(teamID,now)
    if now then
        ...
        rematchRedux.timer:Start(0,rebuildAllTeams)
    end
end
...
function rematchRedux.rebuild:ValidateTeamID(teamID,now)
