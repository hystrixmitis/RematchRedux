...
rematchRedux.events:Register(rematchRedux.petsPanel,"PLAYER_LOGIN",function(self)
    ...
    self.List:Setup({
        allData = petList,
        normalTemplate = "RematchReduxNormalPetListButtonTemplate",
        normalFill = self.FillNormal,
        normalHeight = 44,
        compactTemplate = "RematchReduxCompactPetListButtonTemplate",
        ...
        onScroll = function(self,percent) if not rematchRedux.menus:IsMenuOpen("PetFilterMenu") then rematchRedux.menus:Hide() end end
    })
    ...
    if not rematchRedux.filters:IsClear("Search") or not rematchRedux.filters:IsClear("Stats") then
    ...
    -- rematchRedux.petsPanel.Top.TypeBar.Level25Button.tooltipTitle = L["Max Level Filter"]
    -- rematchRedux.petsPanel.Top.TypeBar.Level25Button.tooltipBody = format(L["%s Level 25 pets\n%s Level 25 \124cff2090fdrare\124r pets"],C.LMB_TEXT_ICON,C.RMB_TEXT_ICON)

    rematchRedux.events:Register(self,"REMATCHREDUX_PETS_CHANGED",self.Update)
end)

function rematchRedux.petsPanel:Configure()
...
function rematchRedux.petsPanel:Update()
    wipe(petList)
...
function rematchRedux.petsPanel:Refresh()
    self.List:Select("Summoned",C_PetJournal.GetSummonedPetGUID(),true)
    self.List:Refresh()
end

-- while petsPanel on screen, refresh the list when a pet is summoned/dismissed for the summoned yellow overlay
function rematchRedux.petsPanel:OnShow()
    rematchRedux.events:Register(self,"COMPANION_UPDATE",self.Refresh)
end

function rematchRedux.petsPanel:OnHide()
    rematchRedux.events:Unregister(self,"COMPANION_UPDATE")
end

-- callback to fill normal-sized petlistbuttons
function rematchRedux.petsPanel:FillNormal(petID)
    self:Fill(petID)
end

-- callback to fill compact-sized petlistbuttons
function rematchRedux.petsPanel:FillCompact(petID)
    self:Fill(petID)
end

-- click of toggle button in topleft to open/close typebar
function rematchRedux.petsPanel.Top.ToggleButton:OnClick()
    settings.UseTypeBar = not settings.UseTypeBar
    rematchRedux.petsPanel:Configure()
    PlaySound(C.SOUND_HEADER_CLICK)
end

-- click of filter button in topright to summon filter menu
function rematchRedux.petsPanel.Top.FilterButton:OnClick(button)
    if rematchRedux.dialog:GetOpenDialog()~="PetHerder" then
        rematchRedux.dialog:HideDialog()
    end
    rematchRedux.menus:Toggle("PetFilterMenu",self)
end

--[[ typebar ]]

local tabInfo = {multiCheck=10} -- for use in petFilterMenu methods, reused table to define group/key
function rematchRedux.petsPanel.Top.TypeBar:Update()
    ...
    for i,tab in ipairs(self.Tabs) do
        ...
        local hasStuff = rematchRedux.petFilterMenu.GroupUsed(tabInfo)
    ...
    for i=1,10 do
        tabInfo.key = i
        local isSelected = rematchRedux.petFilterMenu.GetChecked(tabInfo)
        ...
        self.Buttons[i]:SetDesaturated(rematchRedux.petFilterMenu.GroupUsed(tabInfo) and not isSelected)
    end
    -- update Level25Button, highlighted when only the Level filter is max level (key=4)
    if rematchRedux.filters:HasJust("Level",4) then
        self.Level25Highlight:Show()
        self.Clear:Show()
        if rematchRedux.filters:HasJust("Rarity",4) then -- and if rare selected, make level 25 blue
...
function rematchRedux.petsPanel.Top.TypeBar:TypeButtonOnClick(button)
    ...
    rematchRedux.petFilterMenu.ToggleChecked(tabInfo)
    rematchRedux.menus:Hide()
end

-- the level 25 button will toggle a Level filter just for max-level (key=4)
function rematchRedux.petsPanel.Top.TypeBar.Level25Button:OnClick(button)
    local isFilteringLevel25 = rematchRedux.filters:HasJust("Level",4) -- true if 4th level option (max level) set
    local isFilteringRare = rematchRedux.filters:HasJust("Rarity",4) -- true if 4th rarity option (rare) set
    rematchRedux.filters:Clear("Level")
    rematchRedux.filters:Clear("Rarity")
    if button=="RightButton" and not (isFilteringLevel25 and isFilteringRare) then
        rematchRedux.filters:Set("Level",4,true)
        rematchRedux.filters:Set("Rarity",4,true)
...
function rematchRedux.petsPanel.Top.TypeBar.Clear:OnClick()
    tabInfo.group = typebarTabInfo[settings.TypeBarTab][5] -- current tab's group: Types, Strong or Tough
    if rematchRedux.petFilterMenu.GroupUsed(tabInfo) then
        rematchRedux.petFilterMenu.ResetGroup(tabInfo)
    else
        for i=1,3 do
...
function rematchRedux.petsPanel.Top.SearchBox:OnTextChanged(userInput)
    local text = self:GetText()
    -- only run a filter if the user made the change
    if userInput then
        rematchRedux.filters:SetSearch(text)
        rematchRedux.petsPanel:Update()
    end
...
function rematchRedux.petsPanel.ResultsBar:Update()
    self.NumPets:SetText(format(L["Pets: %s%d"],C.HEX_WHITE,#petList))
    local filterList,onlySearch = rematchRedux.filters:GetFilterList()
...
function rematchRedux.petsPanel.ResultsBar.Clear:OnClick()
