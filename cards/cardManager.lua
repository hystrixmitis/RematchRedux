...
--[[
    ...
    To use, register a card's frame (such as rematchRedux.petCard) with functions for updating, locking and (if possible) pinning:

        rematchRedux.cardManager:Register(cardname,frame,{
        ...
        -- an example with self as rematchRedux.petCard
    ...
    unlocked. Registering will do that automatically, but if you add new elements after the card is registered:
        ...
        rematchRedux.cardManager:AddClickableElementToCard(frame,element)

    Now in your OnEnter/OnLeave/OnClick script handlers for the pet/buttons, call the appropriate function here:

        rematchRedux.cardManager:OnEnter(frame,relativeTo,subject)
        rematchRedux.cardManager:OnLeave(frame)
        rematchRedux.cardManager:OnClick(frame,relativeTo,subject)

        In the above:
        - frame is the card's frame, such as rematchRedux.petCard
...
-- eg: rematchRedux.cardManager:AddAnchorException(rematchRedux.petCard,PetBattleFrame.ActiveAlly,"TOPRIGHT",PetBattleFrame.ActiveAlly,"BOTTOMRIGHT")
...
--function rematchRedux.cardManager:Register(cardname,frame,update,lockUpdate,pinUpdate,shouldShow,noAnchor)
function rematchRedux.cardManager:Register(cardname,frame,def)
    ...
    cardInfo[frame] = {
        ...
        timer = function() -- rematchRedux.timer func to display the card on a short delay (C.CARD_MANAER_DELAY)
            if cardInfo[frame].enteredSubject then
                frame:Show()
                def.update(frame,cardInfo[frame].enteredSubject)
                rematchRedux.cardManager:UnlockCard(frame)
                rematchRedux.cardManager:AnchorCard(frame)
            end
        end
    }
    ...
    frame.escButton:SetScript("OnKeyDown",function(self,key) rematchRedux.cardManager.OnKeyDown(self,self:GetParent(),key) end)

    rematchRedux.cardManager:AddClickableElements(cardInfo[frame].clickableElements,frame) -- populates clickableElements
    rematchRedux.cardManager:UnlockCard(frame,true)
...
function rematchRedux.cardManager:AddClickableElements(clickableElements,frame)
    if frame:IsMouseEnabled() then
        tinsert(clickableElements,frame)
    end
    for _,child in pairs({frame:GetChildren()}) do
        rematchRedux.cardManager:AddClickableElements(clickableElements,child)
    end
end

-- intended for use after a card is registered, this adds the given element to the card frame's clickableElements
function rematchRedux.cardManager:AddClickableElementToCard(frame,element)
...
--   "Click": Nothing
function rematchRedux.cardManager:OnEnter(frame,relativeTo,subject)
    ...
    if behavior==C.MOUSE_SPEED_SLOW and frameNotLocked then
        rematchRedux.timer:Start(C.CARD_MANAGER_DELAY_SLOW,info.timer)
    elseif behavior==C.MOUSE_SPEED_NORMAL and frameNotLocked then
        rematchRedux.timer:Start(C.CARD_MANAGER_DELAY_NORMAL,info.timer)
...
--   "Click": Nothing
function rematchRedux.cardManager:OnLeave(frame)
    local behavior = settings.CardBehavior
    local info = cardInfo[frame]
    ...
    if behavior==C.MOUSE_SPEED_NORMAL or behavior==C.MOUSE_SPEED_SLOW then
        if not frame:IsVisible() and rematchRedux.timer:IsRunning(info.timer) then
            rematchRedux.timer:Stop(info.timer)
...
function rematchRedux.cardManager:OnClick(frame,relativeTo,subject)
    local behavior = settings.CardBehavior
    local info = cardInfo[frame]
    ...
    if frame==rematchRedux.petCard and rematchRedux.utils:HandleSpecialPetClicks(subject) then
    ...
    if behavior==C.MOUSE_SPEED_NORMAL or behavior==C.MOUSE_SPEED_SLOW or behavior==C.MOUSE_SPEED_FAST then
        ...
        if not isVisible then
            if rematchRedux.timer:IsRunning(info.timer) then
                rematchRedux.timer:Stop(info.timer)
            end
            ...
            rematchRedux.cardManager:LockCard(frame)
            rematchRedux.cardManager:AnchorCard(frame)
...
function rematchRedux.cardManager:OnKeyDown(frame,key)
    local info = cardInfo[frame]
    if info.locked and key==GetBindingKey("TOGGLEGAMEMENU") and not rematchRedux.utils:Evaluate(info.noEscape,self,info.lockedSubject) then
        frame:Hide()
...
function rematchRedux.cardManager:LockCard(frame,force)
    local info = cardInfo[frame]
...
function rematchRedux.cardManager:UnlockCard(frame,force)
    local info = cardInfo[frame]
...
function rematchRedux.cardManager:Unpin(frame)
    local info = cardInfo[frame]
...
function rematchRedux.cardManager:SetItemRefMode(frame)
    cardInfo[frame].itemRefMode = true
end

-- when an unpinned card should anchor to a specific anchor, add an exception here
-- eg: rematchRedux.cardManager:AddAnchorException(rematchRedux.petCard,PetBattleFrame.ActiveAlly,"TOPRIGHT",PetBattleFrame.ActiveAlly,"BOTTOMRIGHT")
function rematchRedux.cardManager:AddAnchorException(frame,relativeTo,...)
...
function rematchRedux.cardManager:AnchorCard(frame)
    local info = cardInfo[frame]
    ...
    elseif rematchRedux.cardManager:IsCardPinned(frame) and settings[info.savedvarXPos] and settings[info.savedvarYPos] then
        ...
        elseif relativeTo then
            local corner,opposite = rematchRedux.utils:GetCorner(rematchRedux.utils:GetFrameForReference(relativeTo),UIParent)
...
function rematchRedux.cardManager:HideCard(frame)
    local info = cardInfo[frame]
    -- if card is waiting to be shown, stop waiting
    if info.timer and rematchRedux.timer:IsRunning(info.timer) then
        rematchRedux.timer:Stop(info.timer)
    end
    frame:Hide()
end
...
function rematchRedux.cardManager:HideAllCards()
    for frame,info in pairs(cardInfo) do
        if not rematchRedux.utils:Evaluate(info.noHide,self,info.lockedSubject) then
            rematchRedux.cardManager:HideCard(frame)
        end
    end
end

-- when a card should be shown and locked without an OnEnter/OnLeave/OnClick
function rematchRedux.cardManager:ShowCard(frame,subject)
    local info = cardInfo[frame]
    rematchRedux.cardManager:HideCard(frame)
    info.lockedSubject = subject
    rematchRedux.cardManager:OnClick(frame,rematchRedux.frame,subject) -- rematchRedux window used as reference
end
